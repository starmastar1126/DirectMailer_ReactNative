...
import { getState, dispatch } from '../redux/utils';
...
const RocketChat = {
    ...
    connect(_url) {
        return new Promise((resolve) => {
            ...
            Meteor.ddp.on('connected', () => {
                ...
                //                 _id: item._id
                //             };
                //             setting._server = { id: getState().server.server, current: false };
                ...
                //         });
                //     });
                //     dispatch(actions.setAllSettings(settings));
                // });
                ...
                Meteor.ddp.on('changed', (ddbMessage) => {
                    if (ddbMessage.collection === 'stream-room-messages') {
                        realm.write(() => {
                            ...
                            message.temp = false;
                            message._server = { id: getState().server.server, current: true };
                    ...
                    if (ddbMessage.collection === 'stream-notify-user') {
                        realm.write(() => {
                            ...
                            data._server = { id: getState().server.server, current: true };
            ...
            if (data.length) {
                realm.write(() => {
                    data.forEach((subscription) => {
                        ...
                        // }
                        subscription._server = { id: getState().server.server, current: true };
    ...
    getMessage(rid, msg = {}) {
        const _id = Random.id();
        // console.log(getState().server.server)
        // console.log('getState().login.id ', getState().login);
        const message = {
            _id,
            rid,
            msg,
            ...
            _updatedAt: new Date(),
            temp: true,
            _server: { id: getState().server.server, current: true },
            u: {
                _id: getState().login.user.id || '1',
                username: getState().login.user.id
            }
        };
